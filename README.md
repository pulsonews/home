...
## 9. Só notícias com foto

Em **Admin → Visão geral**, marque "Importar somente notícias com foto".
A partir da próxima atualização de RSS, itens sem imagem detectável são
descartados na importação. Isso não remove retroativamente notícias sem
foto já salvas — elas somem naturalmente conforme envelhecem e saem do
limite de 500 artigos guardados.

## 10. Matérias autorais com IA

Em **Admin → Matérias autorais**, escolha uma notícia agregada, selecione
o motor de IA (**Claude** ou **Gemini**) e clique em "Gerar matéria
autoral". O site chama a API escolhida para escrever um texto
**original**, com apuração própria baseada no resumo da fonte — sem
copiar frases e sem inventar fatos (o prompt do sistema proíbe
explicitamente invenção de dados, citações ou eventos, e é **idêntico**
para os dois motores, garantindo o mesmo padrão de qualidade e precisão).

Requisitos:
- Pelo menos uma das variáveis de ambiente:
  - `ANTHROPIC_API_KEY` (crie em
    [platform.anthropic.com](https://platform.anthropic.com)) para usar o
    Claude.
  - `GEMINI_API_KEY` (crie em
    [aistudio.google.com/apikey](https://aistudio.google.com/apikey)) para
    usar o Gemini.
- Ambas são contas pagas por uso (o Gemini tem uma cota gratuita
  limitada). Cada geração consome créditos da conta correspondente.

A matéria gerada:
- Aparece com um selo dourado "Autoral" nos cards.
- Mostra um aviso de transparência no topo da página, informando que foi
  elaborada com apoio de IA e linkando a apuração original.
- Nunca é apagada pela limpeza automática de notícias antigas (só as
  agregadas via RSS são podadas depois de 500 registros).

Isso ajuda o SEO porque cria conteúdo genuinamente original no domínio
(o que buscadores como o Google valorizam), em vez de só republicar
resumos de terceiros. Ainda assim, use com responsabilidade: revise as
matérias geradas antes de divulgar amplamente, já que nenhum sistema de
IA é infalível — o prompt reduz drasticamente o risco de invenção de
fatos, mas não elimina a necessidade de revisão editorial humana.
