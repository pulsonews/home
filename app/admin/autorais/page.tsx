...
import DisplayPrefsToggle from "@/components/admin/DisplayPrefsToggle";

export default async function AutoraisPage() {
  if (!isAuthenticated()) return <AdminLogin />;

  const [artigos, mostrarSeloAutoral, mostrarAvisoIA] = await Promise.all([
    ...
    <div className="min-h-screen bg-paper">
      ...
      <main className="mx-auto max-w-5xl px-4 py-10">
        ...
        <p className="font-ui text-charcoal/60 mb-4 max-w-2xl">
          ...
          apuração da fonte, sem copiar frases e sem inventar fatos.
        </p>
        <p className="font-ui text-xs text-charcoal/40 mb-8">
          ...
          (Claude ou Gemini). Escolha o motor e se quer citar a fonte
          original antes de gerar cada matéria.
        </p>

        <div className="border border-line rounded-sm p-6 mb-8">
          <h2 className="font-display text-lg mb-3">Exibição pública</h2>
          <DisplayPrefsToggle
            seloInicial={mostrarSeloAutoral === "true"}
            avisoInicial={mostrarAvisoIA === "true"}
          />
        </div>
