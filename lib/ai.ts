...
async function getChave(chaveConfig: string, envVar: string): Promise<string | null> {
  ...
  if (daBase && daBase.trim()) return daBase.trim();
...
async function gerarComClaude(artigo: Artigo): Promise<Resultado> {
  ...
  if (!apiKey) {
    throw new Error(
      "Chave da Anthropic não configurada. Adicione em Admin → Integrações, ou defina ANTHROPIC_API_KEY."
    );
  }
...
async function gerarComGemini(artigo: Artigo): Promise<Resultado> {
  ...
  if (!apiKey) {
    throw new Error(
      "Chave do Gemini não configurada. Adicione em Admin → Integrações, ou defina GEMINI_API_KEY."
    );
  }
